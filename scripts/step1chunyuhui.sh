...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"chunyuhuiMSP":.[1]}}}}}' config.json ./channel-artifacts/${ORG_ADD_NAME}.json > modified_config.json
set +x
...
echo "========= 接下来peer0.org2-benchi 也签署该事务 ========= "
echo
...
#setGlobals 0 2
#set -x
#peer channel update -f ${ORG_ADD_NAME}_update_in_envelope.pb -c ${CHANNEL_NAME} -o car0.car.com:7050 --tls --cafile ${ORDERER_CA}
#set +x
#
#echo
